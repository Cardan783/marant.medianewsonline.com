...
const myChart = new Chart(ctx, {
  ...
  options: {
    ...
    resizeDelay: 200, // Espera 200ms para redibujar, mejorando el rendimiento al cambiar tamaño
    interaction: {
      mode: "index",
      intersect: false,
    },
...
async function actualizarGrafica(
  ...
  let url = "http://marant.medianewsonline.com/php/obtener_datos.php";
  ...
  if (fechaInicio && fechaFin) {
    ...
    params.append("fecha_inicio", formatearFecha(fechaInicio));
    params.append("fecha_fin", formatearFecha(fechaFin));
  }

  // Enviar solo el umbral de Advertencia/Gráfica
  params.append("umbral_temp", umbralAdvertencia);
  // Agregar timestamp para evitar caché del navegador y forzar nueva petición
  ...
  try {
    ...
    datosSensoresCargados = {
      ...
      max_temp_rango:
        data.max_temp_rango !== null ? parseFloat(data.max_temp_rango) : null,
      hora_max_temp_rango: data.hora_max_temp_rango || null,
      min_presion_rango:
        data.min_presion_rango !== null
          ? parseFloat(data.min_presion_rango)
          : null,
      hora_min_presion_rango: data.hora_min_presion_rango || null,
      max_voltaje_rango:
        data.max_voltaje_rango !== null
          ? parseFloat(data.max_voltaje_rango)
          : null,
      min_voltaje_rango:
        data.min_voltaje_rango !== null
          ? parseFloat(data.min_voltaje_rango)
          : null,
    };

    let currentTemp =
      data.temperatura_actual !== undefined && data.temperatura_actual !== null
        ? parseFloat(data.temperatura_actual)
        : datosSensoresCargados.temperatura.slice(-1)[0] || null;

    let currentPresion =
      data.presion_actual !== undefined && data.presion_actual !== null
        ? parseFloat(data.presion_actual)
        : datosSensoresCargados.presion.slice(-1)[0] || null;

    let currentVoltaje =
      data.voltaje_actual !== undefined && data.voltaje_actual !== null
        ? parseFloat(data.voltaje_actual)
        : datosSensoresCargados.voltaje.slice(-1)[0] || null;
